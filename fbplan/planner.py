...
class PlannerConfig:
    ...
    Attributes:
        ...
        min_commit_steps: сколько шагов МИНИМУМ держаться за подцель, если она
            ещё не достигнута. Замер показал 26-31 смену подцели за эпизод:
            низкоуровневая политика управляет локомоцией, и цель, меняющаяся
            каждые несколько шагов, не даёт муравью набрать ход. Достижение
            подцели этот порог не блокирует.

...
* **Пессимистичный отбор.** Брать не точечную оценку, а её нижнюю границу по
  ансамблю: сейчас головы сводятся минимумом меры (пессимизм к достижимости), но
  при последующем `argmin` по узлам этого не хватает. Проверяется заменой одной
  строки в `fb_api._pairwise_min_measures` и прогоном тех же скриптов.

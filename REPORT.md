...
Все числа выше получены на CPU с урезанной конфигурацией (300 узлов, 8 членов),
и это была очевидная слабость: диагноз гласил «узкое место — качество попарных
оценок», а мерялось на заведомо худших рёбрах, чем метод позволяет. Прогон на
GPU (Colab, T4) закрывает вопрос. Сиды 1–3, 300 спаренных эпизодов на строку.
...
Абляция глубины плана на лучших рёбрах (1000 узлов, 32 члена):

| хвост | успех |
|---|---|
| Дейкстра | 0.430 [0.420, 0.440] |
| одна подцель (на CPU) | 0.690 |

Главный вывод 5.6 воспроизводится и при максимальном качестве рёбер: 0.430
против 0.700 у плана из одной подцели.

**Оговорка о чистоте этого сравнения.** Прогон с `direct` на полной
конфигурации остановился — кончилась квота GPU. Число 0.700 в таблице выше
взято из строки «1000 узлов, 32 члена», а там дополнительно включён запас
доверия (`--plan_advantage_steps 25`), которого у ветки с Дейкстрой нет. То есть
на GPU эти две ветки отличаются двумя вещами, а не одной. Чистая абляция есть
только на CPU (0.47 против 0.69, обе без запаса), и она даёт тот же ответ той
же величины. Досчитать недостающую ветку — единственное, что осталось.

**Ещё одна оговорка, про воспроизводимость.** Та же конфигурация (300 узлов,
8 членов, сиды 1–3) дала на CPU −0.067 [−0.080, −0.050], а на GPU −0.020
...
но заявлять «проигрыш ровно −0.067» было бы неосторожно: величина разрыва
плавает в пределах нескольких процентных пунктов.

### 5.10 Смещение отбора: почему корреляция была неправильной метрикой

Раз улучшение рёбер не помогает, проверим то, что до сих пор молча
предполагалось: что хорошая корреляция оценки с истиной означает пригодность
оценки для `argmin`. Это разные вещи, и разница измерима.

Для 200 случайных состояний посчитаем ошибку оценки `оценка − истина` в шагах
среды: отрицательная означает, что FB считает узел ближе, чем он есть. Сравним
...
**Отсюда видно, почему я так долго шёл не туда.** Весь раздел 5.3 мерил
корреляцию и по ней принимал решения. Корреляция росла (0.18 → 0.70), метод не
работал — и это не противоречие: агрегация по набору действительно улучшает
оценку в среднем, что таблица выше и подтверждает (смещение падает с −346 до
−66). Но даже лучшая из полученных оценок остаётся непригодной для отбора из
сотен кандидатов.
...
от маршрута запас в 25 шагов, мы грубо вычитаем часть смещения. Порог
подбирался эмпирически и работал — теперь понятно, что именно он компенсировал.

Что из этого следует делать. Улучшать точность оценок — не тот рычаг: и
корреляция уже неплоха, и смещение при 16 членах втрое меньше, чем при 1, а
метод всё равно проигрывает. Лечить надо **оператор выбора**:
...
Обе идеи проверяемы теми же скриптами и не требуют переобучения. Это и есть
осмысленное продолжение работы — в отличие от наращивания глубины плана.

**Оговорка.** Замер сделан на локальном графе (500 узлов, 16 членов) и на одной
среде. Он объясняет направление и порядок величины, но не доказывает, что
смещение отбора — единственная причина проигрыша.

### 5.11 Пессимистичный отбор: проверено, не работает

Раздел 5.10 заканчивался предложением лечить не оценку, а оператор выбора: брать
не точечную стоимость, а её пессимистичную границу. Идея реализована
(`--disagreement_penalty`) и проверена — и **не подтвердилась**.

Сначала уточнение, потому что «добавить пессимизм» звучит так, будто его не
было. Он есть: ансамбль сводится минимумом меры, то есть максимумом стоимости,
и узел считается достижимым, только если в это верят обе головы. Поэтому
проверялся более сильный вариант — штраф за расхождение голов:
...
поведение прежнее (проверено: совпадает побитово).

Замер на 200 состояниях и 300 узлах. Ключевой столбец — последний: он говорит,
насколько на самом деле далёк узел, который выбрал планировщик. Именно его и
надо минимизировать, всё остальное — промежуточные величины.
...
Это уточняет и направление дальнейшей работы. Раз выбор подцели сам по себе
разумен, чинить надо не его, а калибровку: нужен способ пересчитать `c(s → w)`
в честные шаги без привилегированных координат. Прогонов в среде для проверки
этой ветки не делалось — офлайн-замер показал улучшение в пределах шума, и
тратить на него час GPU было бы необоснованно.

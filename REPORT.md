...
В код я добавил защиту `_warn_if_edges_degenerate`: если более половины рёбер
...
Это ограничение синтетических тестов, которое я осознал только постфактум.
...
Однако та же величина для цели задачи коррелировала на 0.752. Различие я нашёл
...
Альтернативу я проверил и отверг: усреднение `B` по окну траектории из 16 шагов
...
Отдельно я измерил режим исполнения. В первых версиях подцель шла напрямую в
...
**Эти числа я пересчитал после исправления воспроизводимости** (раздел 3). До
...
Поскольку улучшение рёбер не помогло, я решил проверить допущение, которое до
...
Гипотеза, с которой я начинал работу, — «одношаговый контроллер упирается в
...
**Общая оценка работы.** Цель не достигнута; при этом я считаю, что доведение

...
> ошибку.
>
> Прогон на GPU уточнил и это (5.9). Рёбра лучшего качества разрыв **не
> сокращают, а увеличивают**: с −0.020 при 300 узлах и 8 членах до −0.093 при
> 1000 узлов и 32 членах. Значит дело не в точности оценок, а в **числе
> кандидатов**: минимум по большему набору зашумлённых величин смещён сильнее,
> и поиск превращает случайную ошибку в систематическую. Ответ на открытый
> вопрос статьи: наращивать глубину плана бессмысленно, пока смещён сам
> оператор выбора.
>
...
### 5.9 Проверка на GPU: качество рёбер не спасает, а вредит
...
**Гипотеза опровергнута.** Рёбра лучшего качества не сократили разрыв, а
увеличили его вчетверо: с −0.020 (интервал накрывает ноль, то есть ничья) до
−0.093 (значимый проигрыш). Больше узлов и больше членов в наборе — хуже
результат.

Это не просто отрицательный ответ, это **подпись механизма**. Если бы дело было
в разрешении оценки, рост числа членов с 8 до 32 помогал бы: корреляция с
истинным расстоянием растёт 0.455 → 0.528. Вместо этого решает другое —
**число кандидатов в argmin**. При 1000 узлов вместо 300 минимум берётся по
втрое большему набору зашумлённых оценок, а минимум по шумной выборке смещён
тем сильнее, чем выборка больше. Планировщик систематически находит самый
«удачно ошибшийся» узел, и чем богаче граф, тем удачнее эта ошибка.

Отсюда уточнение вывода раздела 5.6. Композиция вредит не потому, что оценки
недостаточно точны, а потому, что **поиск минимума превращает случайную ошибку
в систематическую**, и цена растёт с размером пространства поиска. Улучшать
оценки бессмысленно, пока сам оператор выбора смещён: нужен либо
пессимистичный отбор, либо ограничение числа кандидатов.

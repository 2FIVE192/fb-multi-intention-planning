...
−5.0 п.п. Ради этого отложенные сиды и заводились.

Любопытно, что задачи 3 и 4 — с наибольшим отношением обхода к прямой (4.2 и
2.5) — единственные, где планировщик выигрывает. Это согласуется с предсказанием
P2. Но радоваться рано: разброс по задачам при 60 эпизодах порядка 0.06, а
здесь ещё и знак меняется на соседних задачах с одинаковым отношением 1.4
(задача 1 против задачи 2 — от −0.067 до −0.250). Так что P2 подтверждается по
направлению, но не настолько устойчиво, чтобы на нём что-то строить.

Планировщик к тому же медленнее: средняя длина эпизода 624 шага против 563 у
бейзлайна.

**Эти числа пересчитаны после исправления воспроизводимости** (раздел 3). До
...
изменился, но сдвиг в 3–4 п.п. показывает, во сколько обходилась эта ошибка.
...
давала +0.15 к бейзлайну. На отложенных сидах она даёт лишь +0.033, и
